...
export default function CreateTaskScreen() {
  ...
  const [notificationMinutes, setNotificationMinutes] = useState<number[]>([60, 1440]); // Default: 1 hour and 1 day before
  ...
  const handleCreateTask = async () => {
    ...
    try {
      ...
      await addTask({
        ...
        notificationMinutes: deadline ? notificationMinutes : undefined,
      });
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <DateTimePicker
          label={t('labels.deadline') || 'Deadline'}
          value={deadline}
        ...
        <NotificationPicker
          label={t('labels.reminders') || 'Reminders'}
          value={notificationMinutes}
          onChange={setNotificationMinutes}
          disabled={!deadline}
        />

        {/* Task repeater temporarily hidden */}
        {false && (
          <TouchableOpacity
