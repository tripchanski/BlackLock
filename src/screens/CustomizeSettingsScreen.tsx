...
import { ThemeMode, ThemeColor, FolderViewMode } from '../types';
...
export default function CustomizeSettingsScreen() {
  ...
  const folderViews: { value: FolderViewMode; labelKey: string; icon: string }[] = [
    { value: 'list', labelKey: 'listView', icon: 'list' },
    { value: 'grid', labelKey: 'gridView', icon: 'grid' },
  ];
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.sectionTitle, { color: theme.text }]}>{t('labels.folderView') || 'Folder View'}</Text>
      <View style={styles.modeContainer}>
        {folderViews.map((view) => (
          <TouchableOpacity
            ...
            style={[
              ...
              settings.folderView === view.value && {
                borderColor: theme.primary,
                backgroundColor: theme.primaryLight + '20',
              },
            ]}
            onPress={() => setSettings({ folderView: view.value })}
          >
            <Ionicons
              name={view.icon as any}
              size={32}
              color={settings.folderView === view.value ? theme.primary : theme.textSecondary}
            />
            <Text
              style={[
                styles.modeText,
                { color: theme.textSecondary },
                settings.folderView === view.value && { color: theme.primary },
              ]}
            >
              {t(`labels.${view.labelKey}`) || view.labelKey}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
